...
'@
    ...
    Write-Host "`nXperDeployment Utility Program`nBe sure to run as admin!" -ForegroundColor Green -BackgroundColor Black
    Write-Host "Options:`n[1] Install Apps`n[2] Master Deployment Script`n[3] Retrieve Agent ID`n[4] Retrieve Windows Activation Key`n[5] Configure Desktop and Taskbar`n[6] Get AdBlock Plus Browser Extension`n[7] Activate Bitlocker (Requires reboot)`n[8] Disable Enrollment Checks on First Sign In (will sign out!)`n[9] Exit"
    [int]$MenuSelection = Read-Host "Please enter your selection by number"
    switch ($MenuSelection)
    {
        1{ 
            while ($true) {
                Write-Host "`n1. Exit to Main Menu`n2. O365 (for Business - English)`n3. Ninite (7-Zip, Chrome, Adobe Reader)`n4. New Teams`n5. Google Earth Pro (not silent!)"
                [int]$SubmenuSelection = Read-Host "Please enter your selection by number"
                switch ($SubmenuSelection) {
                    1 { Clear-Host; return }
					2 { InstallO365 ; Clear-Host }
                    3 { InstallNinite; Clear-Host }
					4 { InstallNewTeams; Clear-Host }
					5 { InstallGoogleEarthPro; Clear-Host }
                    default {                        
		...
		2{ 
            while ($true) {
                Write-Host "`nMDS is used to perform universal setup tasks. Please make a selection:`n1. Exit to Main Menu`n2. Create and Run Scripts`n3. Create Scripts"
                [int]$SubmenuSelection = Read-Host "Please enter your selection by number"
                switch ($SubmenuSelection) {
                    1 { Clear-Host; return }
					2 { RunMDS ; Clear-Host ; return }
		...
		2{ RunMDS; Clear-Host}
        3{ PullAgentID; Clear-Host}
        4{ PullWindowsKey; Clear-Host}
        5{ ConfigureDesktopAndTaskbar; Clear-Host}
        6{ GetAdblock; Clear-Host}
        7{ ActivateBitlocker; Clear-Host}
        8{ DisableEnrollmentChecksOnSignIn; Clear-Host}
        9{ Exit 0 }
        default {
            Write-Host "Invalid selection. Please try again."
        }
...
function InstallApps {
    # Placeholder function for installing apps
